...
description: Record of the client-reactjs adoption of the shared instruction set — what was found, what was built, and what was proposed but not applied.
---
...
* `.agents/index/` — `root-index.md` (with the empty override table), `agents-index.md`,
  `agent-wiki-index.md`, `project-wiki-index.md`, `memory-index.md`, `logs-index.md`.
...
* `wiki/information/overview.md`, `wiki/environments/setup.md`,
  `wiki/environments/docker.md`, `wiki/logs/0/16/0/CHANGELOG.md`.
...
* Documentation relocated into the mandated layout — `DESIGN.md` to
  `wiki/information/design-system.md`, and the four loose `wiki/*.md` pages into
...
* `INDEX.md`. Forbidden by the shared directory architecture; its routing role belongs to
  `.agents/index/`.

## Decisions

* **License and version untouched.** Proprietary, LXTranslator organization, already in
  `LICENSE`. `package.json` stays at 0.16.0 — a version bump needs explicit user approval.
* **`.agents/security/` kept local.** Ten files, at the split threshold and not over it,
  so they are listed inline in `agents-index.md` rather than earning a child index.
...
Eight security topics exist in both this repository and
`LXTranslator/server-expressjs` under the same filenames with different bodies:
`authentication-failures`, `broken-access-control`, `exceptional-conditions`,
`secrets-management`, `secure-file-upload`, `security-misconfiguration`,
`sensitive-information-disclosure`, `supply-chain`. The shared directory architecture
lists `security/` as a usually-shared folder, so a shared core with local overrides is
plausible — but it would be a change to `LXAgents/mcp-server`, which is out of scope for
this task and requires the user's approval. Raised as a discovery, not applied.
